...
#define iCeil_SSE(x) _mm_cvt_ss2si(_mm_set_ss(x))

void PLC_calc3(int& c0, int& c1, int& c2, Fvector* P, Fvector& N, light* L, float energy, Fvector& O)
{
...
void CLightShadows::calculate	()
{
	...
	for (u32 o_it=0; o_it<casters.size(); o_it++)
	{
		...
		//	if (p_hat<0.01f)		continue;
			...
			D3DVIEWPORT9 VP{ DWORD(s_x * S_size), DWORD(s_y * S_size), DWORD(S_size), DWORD(S_size), 0.f, 1.f };
	...
	for (u32 s_it=0; s_it<shadows.size(); s_it++)
	{
		...
		for (u32 tid=0; tid<CI->tcnt; tid++)	{
			...
			*/
			...
			PLC_calc3(c0,c1,c2,v,TT.N,S.L,Le,S.C);
